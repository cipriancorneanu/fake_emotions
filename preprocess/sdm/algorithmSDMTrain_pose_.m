function [model] = algorithmSDMTrain_pose(trainData, testData, varargin)
    ...
    function [regressor, results, mae] = regressorSDMTrain(fValues, poseOrig, poseTarg)
        ...
        results = bsxfun(@plus, fValues*regressor.weights, regressor.offsets);
    ...
    for i = 1:T
        ...
        %% Computer and show evaluation metric
        % Compute evaluation metric as mean euclidean distance between
        % predicted landmarks and groundtruth normated by interocular
        % distance
        trainErrors(i+1,:) = squeeze(mean(sqrt((trainShape(:,1,:) - trainShapeTargets(:,1,:)).^2 + ...
            (trainShape(:,2,:) - trainShapeTargets(:,2,:)).^2)));
        norm_train = squeeze(sqrt((trainShapeTargets(14,1,:) - trainShapeTargets(16,1,:)).^2 + ...
                (trainShapeTargets(14,2,:) - trainShapeTargets(16,2,:)).^2))'; 
        trainErrors(i+1,:) = trainErrors(i+1,:)/norm_train;                

        testErrors(i+1,:) = squeeze(mean(sqrt((testShape(:,1,:) - testShapeTargets(:,1,:)).^2 + ...
            (testShape(:,2,:) - testShapeTargets(:,2,:)).^2)));
        norm_test = squeeze(sqrt((testShapeTargets(14,1,:) - testShapeTargets(16,1,:)).^2 + ...
                (testShapeTargets(14,2,:) - testShapeTargets(16,2,:)).^2))';
        testErrors(i+1,:) = testErrors(i+1,:)./norm_test; 
        
        disp(['Training error at T=' num2str(i) ': ' num2str(mean(trainErrors(i+1,:)))]);
        disp(['Testing error at T=' num2str(i) ':  ' num2str(mean(testErrors(i+1,:)))]);
    end
end